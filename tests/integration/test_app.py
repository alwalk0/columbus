...
def create_test_database():
    ...
    data = [{'id': 1, 'name': 'boo', 'breed': 'labrador', 'age': 3},{'id': 2, 'name': 'moo', 'breed': 'terrier', 'age': 5}]
...
def test_post(client):
    response = client.post('/dogs', content=json.dumps({'id': 3, 'name': 'fifi', 'breed': 'labrador', 'age': 3}))
    print(response.text)
    assert response.status_code == 200 
