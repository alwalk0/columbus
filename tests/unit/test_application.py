...
def views_config(mock_table, mock_database):
    views_config = {'method': 'POST', 'url': '/dogs', 'database': mock_database, 'table': mock_table}
    yield views_config
...
def test_create_view_dict(mocker, mock_table, mock_database):
    ...
    view_dict = create_view_dict(method='POST', url='/dogs', database=mock_database, table=mock_table, models='models.py')
