...
def test_put_request_query(mock_table):
    ...
    assert (
        query == "UPDATE table SET foo = :foo, bar = :bar, test = :test WHERE id = :id"
    )


def test_create_error_message_one_key():
    error_message = create_error_message(
        message="These errors have occured", keys=["foo"]
    )
    assert error_message == "These errors have occured: foo"


def test_create_error_message_more_keys():
    error_message = create_error_message(
        message="These errors have occured", keys=["foo", "bar"]
    )
    assert error_message == "These errors have occured: foo, bar"
